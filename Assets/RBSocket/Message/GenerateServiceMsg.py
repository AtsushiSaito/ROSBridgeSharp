...
class CHClassGenerate():
    def __init__(self):
        ...
        self.CommandServiceShowRun()
        self.Split_RequestResponse()

        # Request
        self.CommandRowData = self.RequestRowData
        self.msg_name = self.request_name
        self.CommandRowData_to_NoNestMsgData()
        self.ParseTypeNameSpace()
        self.SplitTypeAndName()
        self.CheckNameList()
        self.RequestGenerateMsgText()

        # Response
        self.CommandRowData = self.ResponseRowData
        self.msg_name = self.response_name
        self.CommandRowData_to_NoNestMsgData()
        self.ParseTypeNameSpace()
        self.SplitTypeAndName()
        self.CheckNameList()
        self.ResponseGenerateMsgText()
    ...
    def set_msg_name(self, msg_arg):
        ...
        self.request_name = str(msg_arg.rsplit("/", 1)[1]) + "Request"
        self.response_name = str(msg_arg.rsplit("/", 1)[1]) + "Response"
        self.msg_original_name = str(msg_arg.rsplit("/", 1)[1])

    def CommandServiceShowRun(self):
        # rosmsg showコマンドを実行してメッセージ情報を受取る
        self.CommandRowData = subprocess.Popen(
            "rossrv show " + self.msg, stdout=subprocess.PIPE, shell=True).communicate()[0]
    ...
    def Split_RequestResponse(self):
        self.RequestRowData = ""
        self.ResponseRowData = ""
        data = self.CommandRowData.split("\n")  # 改行で分割
        turning_flag = True
        for i in range(len(data)):
            if data[i] == "---":
                ...
                continue
            if turning_flag:
                self.RequestRowData += data[i] + "\n"
            else:
                if i != len(data) - 1:
                    self.ResponseRowData += data[i] + "\n"
        self.RequestRowData += "\n"
    ...
    def CheckNameList(self):
        for i in range(len(self.NameList)):
            if self.NameList[i] == "default":
                self.NameList[i] = "@default"

    def RequestGenerateMsgText(self):
        text = ""
        text += "using System;\n\n"
    ...
    def ResponseGenerateMsgText(self):
        text = self.maked_text
        text += "\n"
        text += self.GS(4) + "[System.Serializable]\n"
        text += self.GS(4) + "public class " + self.msg_name
        text += " : ExtendMessage\n"
    ...
    def Save(self):
        file = open(self.msg_original_name + ".cs", "w")
